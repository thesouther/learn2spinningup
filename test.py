...
def make_board_game_config(action_space_size: int, max_moves: int, dirichlet_alpha: float,
...
def select_child(config: MuZeroConfig, node: Node, min_max_stats: MinMaxStats):
    _, action, child = max((ucb_score(config, node, child, min_max_stats), action, child)
...
def update_weights(optimizer: tf.train.Optimizer, network: Network, batch, weight_decay: float):
    ...
    for image, actions, targets in batch:
        ...
        for prediction, target in zip(predictions, targets):
            ...
            l = (scalar_loss(value, target_value) + scalar_loss(reward, target_reward) +
